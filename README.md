...
| `:UH` | Jump to the `DEFINE_*` gflags definition or C/C++ counter constant definition under cursor. Searches the current file first; from headers, searches the same-name source file before falling back to the repo. `FLAGS_xxx` is resolved as `xxx`. | `:UH` |
